...
// Public re-exports — preserves the original `crate::framework::command::*` API
// so downstream code does not need to change.
pub use config::{AppConfig, AreaConfig, LayoutConfig, ParserConfig, WidgetConfig};
pub use exec::{split_command_args, BoundCommand, CommandRunner};
pub use parser::{LoggedLine, OutputParser, ParsedOutput};
