...
impl Compositor {
    ...
    pub fn render<W: Write>(&mut self, writer: &mut W) -> io::Result<()> {
        ...
        // Pre-computed clear cell to avoid struct construction in loop
        let clear_cell = Cell {
            char: ' ',
            fg: Color::Reset,
            bg: self.clear_color,
        ...
        if full_refresh || regions.is_empty() {
            // Use fill() with a matching clear cell pattern when possible,
        ...
        for y in 0..self.height {
            ...
            for x in 0..self.width {
                ...
                // Write char - fast path for ASCII printable (common case)
                // Only output 0x20-0x7E (printable ASCII), skip control chars
