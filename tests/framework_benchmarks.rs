...
fn bench_event_bus_publish_throughput() {
    ...
    println!(
        "EventBus publish: {} ops/sec ({:?} total)",
        ops_per_sec as u64, elapsed
    );
    assert!(
        ops_per_sec > 1_000_000.0,
        "EventBus publish too slow: {} ops/sec",
        ops_per_sec
    );
}
...
fn bench_event_bus_subscribe_and_publish() {
    ...
    let ops_per_sec = iterations as f64 / elapsed.as_secs_f64();
    println!(
        "EventBus publish+deliver: {} ops/sec ({:?} total)",
        ops_per_sec as u64, elapsed
    );
    assert!(
        ops_per_sec > 100_000.0,
        "EventBus delivery too slow: {} ops/sec",
        ops_per_sec
    );
}
...
fn bench_event_bus_multiple_subscribers() {
    ...
    let ops_per_sec = iterations as f64 / elapsed.as_secs_f64();
    println!(
        "EventBus 10 subscribers: {} ops/sec ({:?} total)",
        ops_per_sec as u64, elapsed
    );
    assert!(
        ops_per_sec > 50_000.0,
        "EventBus multi-subscriber too slow: {} ops/sec",
        ops_per_sec
    );
}
...
fn bench_scene_router_push_pop() {
    ...
    println!(
        "SceneRouter push+pop: {} ops/sec ({:?} total)",
        ops_per_sec as u64, elapsed
    );
    assert!(
        ops_per_sec > 100_000.0,
        "SceneRouter too slow: {} ops/sec",
        ops_per_sec
    );
}
...
fn bench_scene_router_render() {
    ...
    let ops_per_sec = iterations as f64 / elapsed.as_secs_f64();
    println!(
        "SceneRouter render: {} ops/sec ({:?} total)",
        ops_per_sec as u64, elapsed
    );
    assert!(
        ops_per_sec > 10_000.0,
        "SceneRouter render too slow: {} ops/sec",
        ops_per_sec
    );
}
...
fn bench_scene_router_deep_navigation() {
    ...
    println!(
        "SceneRouter deep push (100): {} ops/sec ({:?} total)",
        ops_per_sec as u64, elapsed
    );
    assert!(
        ops_per_sec > 1000.0,
        "SceneRouter deep nav too slow: {} ops/sec",
        ops_per_sec
    );
}
