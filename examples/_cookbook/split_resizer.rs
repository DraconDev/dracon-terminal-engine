...
impl SplitResizerApp {
    ...
    fn render_divider_h(&self, rect: Rect, w1: u16, p: &mut Plane) {
        ...
        for y in 0..rect.height {
            ...
            if idx < p.cells.len() {
                ...
                p.cells[idx] = Cell { char: ch, fg, bg: Color::Ansi(33), style: Styles::empty(), transparent: false, skip: false };
            }
        }
    }
    ...
    fn render_status(&self, rect: Rect, p: &mut Plane) {
        ...
        for (i, c) in text.chars().enumerate().take(rect.width as usize) {
            ...
            if idx < p.cells.len() {
                let fg = if i < sa.len() + sb.len() + 4 { Color::Rgb(0, 255, 136) } else { Color::Rgb(150, 150, 150) };
                p.cells[idx] = Cell { char: c, fg, bg: Color::Ansi(17), style: Styles::BOLD, transparent: false, skip: false };
            }
        }
    }
