...
```"#;
            ...
            if show_help_render.load(Ordering::SeqCst) {
                ...
                for x in hx + 1..hx + hw - 1 {
                    ...
                    if top < plane.cells.len() { plane.cells[top].char = '─'; plane.cells[top].fg = theme.outline; }
                    if bot < plane.cells.len() { plane.cells[bot].char = '─'; plane.cells[bot].fg = theme.outline; }
                }
                for y in hy + 1..hy + hh - 1 {
                    let left = (y * w + hx) as usize;
                    let right = (y * w + hx + hw - 1) as usize;
                    if left < plane.cells.len() { plane.cells[left].char = '│'; plane.cells[left].fg = theme.outline; }
                    if right < plane.cells.len() { plane.cells[right].char = '│'; plane.cells[right].fg = theme.outline; }
                }
                let corners = [('╭', hx, hy), ('╮', hx + hw - 1, hy), ('╰', hx, hy + hh - 1), ('╯', hx + hw - 1, hy + hh - 1)];
                for (ch, cx, cy) in corners.iter() {
                    let idx = (cy * w + cx) as usize;
                    if idx < plane.cells.len() { plane.cells[idx].char = *ch; plane.cells[idx].fg = theme.outline; }
                }
                ...
                for (i, (key, desc)) in shortcuts.iter().enumerate() {
                    ...
                    for (j, c) in key.chars().enumerate() {
                        ...
                        if idx < plane.cells.len() { plane.cells[idx].char = c; plane.cells[idx].fg = theme.primary; }
                    }
                    for (j, c) in desc.chars().enumerate() {
                        let idx = (row * w + hx + 14 + j as u16) as usize;
                        if idx < plane.cells.len() { plane.cells[idx].char = c; plane.cells[idx].fg = theme.fg; }
                    }
                }
            }
