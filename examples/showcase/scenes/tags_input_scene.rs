...
fn tag_categories() -> Vec<TagCategory> {
    vec![
        ...
        TagCategory {
            name: "Scripting",
            icon: '⌨',
            items: &["python", "ruby", "perl", "lua"],
            color: Color::Rgb(208, 135, 112),
        },
        TagCategory {
            name: "Web",
            icon: '◇',
        ...
        TagCategory {
            ...
            color: Color::Rgb(235, 203, 139),
        },
        TagCategory {
            name: "Functional",
            icon: 'λ',
            items: &["haskell", "elixir", "swift"],
    ...
    fn render(&self, area: Rect) -> Plane {
        ...
        if tags.is_empty() {
            draw_text(
                ...
                main_x + 2,
                tags_y + 3,
                "(no tags — type above or click a category)",
                t.fg_muted,
                t.bg,
                false,
            );
            ...
            for (i, tag) in tags.iter().enumerate() {
                ...
                if py + pill_h > area.height.saturating_sub(4) {
                    break;
                }
            ...
            if cap_y < area.height.saturating_sub(4) {
                ...
                draw_text(
                    &mut plane, main_x, cap_y, &cap_text, t.fg_muted, t.bg, false,
                );
                ...
                for dx in 0..bar_w {
                    ...
                    if idx < plane.cells.len() {
                        let bar_color = if count >= max_tags {
                            t.warning
                        } else {
                            t.primary
                        };
        ...
        if log_y > tags_y + 12 {
            ...
            for (i, entry) in self.tag_log.iter().rev().take(3).enumerate() {
                let color = if entry.contains("removed") {
                    t.warning
                } else {
                    t.success
                };
                ...
                if col < DIV_X && row > 3 {
                    ...
                    if idx < cats.len() {
                        ...
                        if let Some(item) = cat
                            .items
                            .iter()
                            .find(|it| !current_tags.contains(&it.to_string()))
                        {
                            self.tags_input.add_tag(item.to_string());
                            self.tag_log
                                .push(format!("+ {} added ({})", item, cat.name));
                            self.dirty = true;
                            return true;
                        }
                    }
    ...
    fn render_sidebar(&self, plane: &mut Plane, area: Rect, t: &Theme) {
        ...
        for (i, cat) in cats.iter().enumerate() {
            ...
            for (j, tag) in cat.items.iter().enumerate() {
                let ty = y + 1 + j as u16;
                if ty >= area.height.saturating_sub(2) {
                    break;
                }
