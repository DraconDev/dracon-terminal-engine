...
//!
//! Demonstrates the Calendar widget with date selection and navigation.
...
impl Scene for CalendarScene {
    ...
    fn render(&self, area: Rect) -> Plane {
        ...
        if let Some(ref date) = self.selected_date {
            let status = format!(" Selected: {} ", date);
            let sx = (area.width.saturating_sub(status.len() as u16)) / 2;
            draw_text(&mut plane, sx, area.height.saturating_sub(3), &status, t.success, t.bg, true);
        }
