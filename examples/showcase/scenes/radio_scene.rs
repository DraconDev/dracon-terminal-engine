...
impl RadioScene {
    fn render_help(&self, plane: &mut Plane, area: Rect) {
        ...
        let lines = [
            ("╭──────────────────────────────────────────╮", true),
            ("│       Radio Button Help                  │", true),
            ("├──────────────────────────────────────────┤", true),
            ("│  Up/Down  Change selection in group      │", false),
            ("│  Tab      Switch between groups           │", false),
            ("│  1/2/3    Quick-select option in group    │", false),
            ("│  Click    Select option directly          │", false),
        ...
        for (i, (line, is_border)) in lines.iter().enumerate() {
            ...
            for (j, ch) in line.chars().enumerate() {
                ...
                if px < area.width && ly < area.height {
                    ...
                    if idx < plane.cells.len() {
                        plane.cells[idx].char = ch;
                        plane.cells[idx].fg = if *is_border
                            || "│╭╮├┤╰╯─".contains(ch)
                        {
                            t.outline
                        } else {
