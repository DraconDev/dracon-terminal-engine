...
impl AnimationScene {
    pub fn new(theme: Theme) -> Self {
        ...
        let progress_bar = ProgressBar::new(WidgetId::new(1))
...
impl Scene for AnimationScene {
    ...
    fn render(&self, area: Rect) -> self::Plane {
        ...
        // ── Header ──────────────────────────────────────────────────────────
        draw_text(&mut plane, 2, 0, " Animation Playground ", t.primary, t.bg, true);
        ...
        let ball_y = 2;
        draw_text(&mut plane, main_x, ball_y, "Bouncing Balls", t.primary, t.bg, true);
        ...
        for ball in balls.iter() {
            ...
            if bx > arena_x && bx < arena_x + ball_arena_w - 1
                && by > ball_y && by < ball_y + ball_arena_h
            {
        ...
        for (i, ball) in balls.iter().enumerate() {
            ...
            draw_text(&mut plane, label_x, label_y, ball.easing_name, ball.color, t.bg, false);
        }

        // Section: Sliding Panel
        let panel_y = ball_y + ball_arena_h + 3;
        if panel_y + 7 < area.height.saturating_sub(2) {
            draw_text(&mut plane, main_x, panel_y, "Sliding Panel", t.primary, t.bg, true);
            ...
            if visible_w > 0 {
                ...
                // Panel content
                if visible_w > 4 {
                    let lines = ["┌──────────────┐", "│ Animated     │", "│ Panel slides │", "│ in/out       │", "└──────────────┘"];
        ...
        if widget_y + 6 < area.height.saturating_sub(3) {
            draw_text(&mut plane, main_x, widget_y, "Animated Widgets", t.primary, t.bg, true);
            ...
            blit_to(&mut plane, &ring_plane, (main_x + bar_w + 2) as usize, bar_area_y as usize);

            // Spinner
            let spin_y = bar_area_y + 5;
            if spin_y < area.height.saturating_sub(3) {
                draw_text(&mut plane, main_x, spin_y, "Spinner:", t.fg_muted, t.bg, false);
        ...
        if self.show_help {
            let help_key = self.keybindings.display(actions::HELP).unwrap_or("f1");
            let back_key = self.keybindings.display(actions::BACK).unwrap_or("esc");
            render_help_overlay(&mut plane, area, &self.theme, "Animation Playground — Help", &[
...
impl AnimationScene {
    fn render_sidebar(&self, plane: &mut Plane, t: &Theme, area_h: u16) {
        ...
        let auto_text = if auto_running { "● Running" } else { "○ Paused " };
        for cx in 0..SIDEBAR_W {
        ...
        let auto_fg = if auto_running { t.bg } else { t.fg }; draw_text_clipped(plane, sx + 1, auto_y + 1, auto_text, sx + SIDEBAR_W, auto_fg, auto_bg, true);
