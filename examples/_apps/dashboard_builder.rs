    ...
    fn new(should_quit: Arc<AtomicBool>) -> Self {
        Self {
            ...
            kv_grid: KeyValueGrid::with_id(WidgetId::new(2))
                .separator(": ")
                .bind_command(
                    BoundCommand::new("echo 'Memory:8.2GB|Disk:45%|Network:120Mbps'").refresh(5),
                ),
...
impl Widget for Dashboard {
    ...
    fn render(&self, area: Rect) -> Plane {
        ...
        copy_plane_cells(
            &mut plane,
            &self.status_badge.render(badge_rect),
            badge_rect.x as usize,
            badge_rect.y as usize,
        );
        ...
        copy_plane_cells(
            &mut plane,
            &self.log_viewer.render(log_rect),
            log_rect.x as usize,
            log_rect.y as usize,
        );
        ...
        copy_plane_cells(
            &mut plane,
            &self.streaming.render(stream_rect),
            stream_rect.x as usize,
            stream_rect.y as usize,
        );
...
fn render_separator(plane: &mut Plane, row: u16, width: u16, color: Color) {
    for x in 0..width as usize {
        let idx =
            (row as usize * plane.width as usize + x).min(plane.cells.len().saturating_sub(1));
        plane.cells[idx] = Cell {
            char: '─',
            fg: color,
            bg: plane.cells[idx].bg,
            style: Styles::empty(),
            transparent: false,
            skip: false,
        };
...
fn render_footer_text(plane: &mut Plane, width: u16, footer_y: u16, theme: &Theme) {
    ...
    for (i, c) in controls.chars().enumerate().take(width as usize - offset) {
        let idx = ((footer_y as usize + 1) * plane.width as usize + offset + i)
...
fn main() -> std::io::Result<()> {
    ...
    app.add_widget(
        Box::new(Dashboard::new(should_quit)),
        Rect::new(0, 0, 80, 24),
    );
