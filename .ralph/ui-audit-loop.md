...
- [x] **widget_gallery** → Widget Workshop (sidebar + demo panel + properties)
  ...
  - Hover feedback on list items, keyboard hints per widget
...
- [x] **theme_switcher** → Theme Studio
  ...
  - 3 sections: Widget Preview (8 widgets), Color Palette (12 swatches), Contrast Ratios
  - Fixed borrow checker issue (extract zone_idx first)
...
## Scene Quality (updated after iterations 1-2)

| Scene | Tier | Layout | Widgets | Notes |
|-------|------|--------|---------|-------|
| widget_gallery | →1 | split | 12 | Sidebar + demo + state |
| theme_switcher | →1 | split | 9 | Sidebar + preview + palette + contrast |
...
| accessibility | 1 | split | 3+ | Form + focus rings + a11y tree |
| hud_demo | 1 | layered | 4+ | Game area + HUD |
...
| password_input | 3 | flat | 1 | Password fields |
| progress | 3 | flat | 1 | Progress bars |
| tree_navigator | 3 | flat | 1 | Tree widget |
| cell_pool | 3 | flat | 1 | CellPool widget |
...
## Next: password_input → Login Screen

Target:
- Centered card layout with border
- Real SearchInput (username) + PasswordInput (password) + Checkbox (remember me) + Button (login)
- Error state simulation (wrong password → error message)
- Success state (welcome message)
- Use hover feedback on fields